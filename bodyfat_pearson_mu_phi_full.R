...
# with both mu (mean) and phi (precision) modeled
# The "|" operator separates the formulas for mu and phi
beta_model_full <- betareg(y ~ X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8 |
                           X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8, data = data_full, link = "logit")
...
for (i in 1:nrow(testing_set)){
  ...
  testFCP_pearson <- Vectorize(function(value){
    ...
    beta_model_new <- betareg(y ~ X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8 | 
                                X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8, data = datan, link = "logit")
    
    # Get the pearson residual for combined set and the candidate value
    res_pearson_full <- abs(residuals(beta_model_new, type = "pearson"))
    
    # Compute the full conformal quantile of residuals
    res_pearson_quantile <- sort(res_pearson_full[1:n_full])[k]
    res_pearson_new <- res_pearson_full[n_full+1]     # residual for candidate value
    return(res_pearson_new <= res_pearson_quantile)
  })
  
  # Compute conformal prediction interval
  CPI <- find_true_interval(testFCP_pearson, tol = 1e-4, prediction_temp)
  lower_full[i] <- CPI[1]
  upper_full[i] <- CPI[2]
}
...
testing_set$predicted_y <- predict(beta_model_full, newdata = testing_set, type = "response")
...
ggplot(plot_data_full, aes(x = Index)) +
  ...
  labs(title = "Beta Regression (mu & phi) — Full CP",
       y = "Response (y)", 

...
calibration_set$phi_hat <- predict(beta_model, newdata = calibration_set, type = "precision")
testing_set$phi_hat <- predict(beta_model, newdata = testing_set, type = "precision")

# Compute quantile residuals on the calibration set
calibration_set$quantile_resid <- abs(qnorm(pbeta(
  calibration_set$y,
  shape1 = calibration_set$mu_hat * calibration_set$phi_hat,
  shape2 = (1 - calibration_set$mu_hat) * calibration_set$phi_hat
)))
...
testing_set$lower <- mapply(function(mu, phi) {
  qbeta(pnorm(-q_alpha), shape1 = mu * phi, shape2 = (1 - mu) * phi)
}, testing_set$mu_hat, testing_set$phi_hat)

testing_set$upper <- mapply(function(mu, phi) {
  qbeta(pnorm(q_alpha), shape1 = mu * phi, shape2 = (1 - mu) * phi)
}, testing_set$mu_hat, testing_set$phi_hat)
...
ggplot(plot_data_split, aes(x = Index)) +
  ...
  labs(title = "Beta Method (quantile with mu,phi) Split CP",
       y = "Response (y)",

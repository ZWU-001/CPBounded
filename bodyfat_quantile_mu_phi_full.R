...
for (i in 1:nrow(testing_set)){
  ...
  testFCP_quantile <- Vectorize(function(value) {
    ...
    beta_model_new <- betareg(
          y ~ X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8 | 
            X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8, data = datan, link = "logit")
    
    # Predict the mean (mu) and the precision (phi) for the augmented dataset
    mu_hat_full <- predict(beta_model_new, type = "response")
    phi_hat_full <- predict(beta_model_new, type = "precision")
    
    # Get the quantile residual for combined set and the candidate value
    quantile_resids <- abs(qnorm(pbeta(
        datan$y,
        shape1 = mu_hat_full * phi_hat_full,
        shape2 = (1 - mu_hat_full) * phi_hat_full
      )))
...
testing_set$mu_hat <- predict(beta_model_full, newdata = testing_set, type = "response")
...
ggplot(plot_data_full, aes(x = Index)) +
  ...
  labs(title = "Beta Method (Quantile with mu,phi) Full CP",
       y = "Response (y)",

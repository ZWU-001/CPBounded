...
library(gamlss) # For heteroscedastic transformation modeling
...
# Apply logit transformation to responses for heteroscedastic modeling
training_set$y_trans <- logit(training_set$y)
calibration_set$y_trans <- logit(calibration_set$y)
testing_set$y_trans <- logit(testing_set$y)

################################################################################
# Heteroscedastic Transformation Model (Split Conformal Prediction)
################################################################################

# Fit GAMLSS model to training data with both location and scale modeled
fit_gamlss_split <- gamlss(
  y_trans ~ X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8,
  sigma.formula = ~ X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8,
  data = training_set,
  family = NO()
)

# Predict mean and standard deviation on calibration set
mu_calib <- predict(fit_gamlss_split, newdata = calibration_set, what = "mu", type = "response")
sigma_calib <- predict(fit_gamlss_split, newdata = calibration_set, what = "sigma", type = "response")

# Compute Pearson residuals for the calibration set
residuals_cal <- abs((calibration_set$y_trans - mu_calib) / sigma_calib)

# Compute the split conformal quantile of residuals
level <- ceiling((1 - alpha) * (length(residuals_cal) + 1))
q_alpha <- sort(residuals_cal)[level]

# Predict on the test set
mu_test <- predict(fit_gamlss_split, newdata = testing_set, what = "mu", type = "response")
sigma_test <- predict(fit_gamlss_split, newdata = testing_set, what = "sigma", type = "response")

# Construct logit-space prediction intervals, transform back to (0,1) scale
lower_split <- mu_test - q_alpha * sigma_test
upper_split <- mu_test + q_alpha * sigma_test
lower_bounds_split <- expit(lower_split)
upper_bounds_split <- expit(upper_split)

# Compute coverage and average width of intervals
coverage_split <- mean((y_test_true >= lower_bounds_split) & (y_test_true <= upper_bounds_split))
avg_width_split <- mean(upper_bounds_split - lower_bounds_split)

################################################################################
# Heteroscedastic Transformation Model (Full Conformal Prediction)
################################################################################
...
linmod_full_data <- lm(y_trans ~ X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8, data = full_data)
...
for (i in 1:length(y_test_true)){
  ...
  classic_pred <- predict(linmod_full_data, newdata = testing_set[i, ], interval = "prediction", level = 1 - alpha)
  ...
  testFCP <- Vectorize(function(value){
    ...
    y_aug <- c(full_data$y_trans, value)
    X_aug <- rbind(full_data[, 1:8], testing_set[i, 1:8])
    X_aug$y_aug <- y_aug
    
    fit_aug <- gamlss(
      y_aug ~ X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8,
      sigma.formula = ~ X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8,
      data = X_aug,
      family = NO(),
      control = gamlss.control(trace = FALSE)
    )
    mu_aug <- predict(fit_aug, what = "mu")
    sigma_aug <- predict(fit_aug, what = "sigma")
    resid_all <- abs((y_aug - mu_aug) / sigma_aug)
    resid_full <- resid_all[1:nrow(full_data)]      # residuals for training+calib
    resid_new <- resid_all[length(resid_all)]       # residual for candidate value
    
    # Compute the full conformal quantile of residuals
    q_alpha_full <- sort(resid_full)[full_level]
    return(resid_new <= q_alpha_full)
  })
...
mu_test_pred_split <- expit(mu_test)
mu_test_pred_full <- expit(predict(fit_gamlss_full, newdata = testing_set, what = "mu", type = "response"))

# Create dataframe for plotting results
plot_data <- data.frame(
  Index = 1:length(y_test_true),
  True_y = y_test_true,
  Predicted_split = mu_test_pred_split,
  Predicted_full = mu_test_pred_full,
  Split_Lower = lower_bounds_split,
  Split_Upper = upper_bounds_split,
...
plot_split <- ggplot(plot_data, aes(x = Index)) +
  ...
  labs(title = "Hetero Transformation Method Split CP",
...
plot_full <- ggplot(plot_data, aes(x = Index)) +
  ...
  labs(title = "Hetero Transformation Method Full CP",
       y = "Response (y)", x = "Test Observation (sorted by True y)") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5)) +
  ylim(0,1)
